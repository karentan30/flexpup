...
module.exports = async function handler(req, res) {
  ...
  if (!HUB_SECRET) {
    console.error('[pay/callback] HUB_SECRET_SCENEME not set');
  ...
  if (!orderNo.startsWith('SM')) return res.status(400).json({ error: 'unrecognized order prefix' });
